...
function coachPolishRedirectReply(): string {
  return [
    "紹介の設計（ワークシート）を対話で磨くのは、紹介コーチのチャットでできます。",
    "そちらで「磨きたい」と送ると、項目ごとに深掘りの問いと改善案を出して、その場で保存できます。",
    "→ https://gia2018.com/members/app/coach",
    "（Web専用。Slack / LINE からは磨けません）",
  ].join("\n");
}
...
async function classifyIntent(client: OpenAI, text: string): Promise<Intent> {
  ...
  if (
    ...
    (/(期限|期日|締切|納期|デッドライン)/.test(trimmed) &&
      /(のば|延ば|伸ば|ずら|遅らせ|前倒し|早め|変更|変えて|変えられ)/.test(trimmed))
  ) {
    return "mutate";
  }
  ...
  if (
    /(同じ人|同一人物|重複).{0,12}(統合|まとめ|一つに|ひとつに)|人物.{0,6}(統合|まとめ)|統合して/.test(trimmed) ||
  ...
  if (
    /(紹介(元|先)|紹介された|紹介してもらった|から紹介|を紹介|紹介を?(頼|依頼|お願い)|紹介しといた|紹介しときました|引き合わせ|繋いだ|つないだ)/.test(trimmed) ||
...
- mutate: データ書き換えを直接指示している短文。下記いずれか：
    ...
    * 人物の統合・削除（例:「○○さんと△△さん同じ人、統合して」「○○さん削除して」）
...
# 機能外の依頼への対応（一般論で埋めない）
- 本AIにできない操作（例：紹介ワークシートを対話で磨く・項目を書き換える）を頼まれたら、汎用ノウハウの箇条書きを並べてはいけない。できる場所を1〜2行で案内する：「紹介設計を磨くのは紹介コーチのチャットでできます → https://gia2018.com/members/app/coach」。
...
- get_action_plan の使い方（オンデマンドの能動提案。LINE/Slack どちらでも）：
  ...
  * quickWin があれば「まずこの1件だけ片付けましょう」と最初に背中を押す（溜まる前に処理を促す）。
  * openPromises があれば「○○さんへの『△△』、その後どうなっていますか？」と必ず思い出させる。連絡が滞っていそうなら率直に促す。
  * staleTaskCount が多いときは個別に羅列せず「期限切れが◯件たまっています、棚卸しを」と1行で促す（鳴らし続けない）。
  * 候補が薄い日は正直に「今日は掘り起こし候補が少ないので、新規接点づくり・種まきに使える日です」と伝える。嘘の用事を作らない。
  * 連絡文の下書きが欲しいと言われたら、その相手の名前で search_conversations を引いてから自然な連絡文を提案する。
