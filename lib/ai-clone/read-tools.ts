...
export const aiCloneReadTools: ChatCompletionTool[] = [
  {
    ...
    function: {
      ...
      description:
        ...
        "過去の会話を参照する必要がある時に使う。" +
        "query は要約・本文・次のアクションを横断 ilike 検索。" +
        "person_name 指定で特定人物の会話だけに絞る（部分一致）。" +
        "結果は最大10件、新しい順。",
      parameters: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description: "キーワード。要約・本文・次のアクションに横断 ilike。例：「契約」「旅費」「来週の見積」",
          },
          person_name: {
            type: "string",
            description: "登場人物の名前（部分一致）。指定するとその人との会話だけに絞る",
          },
          date_from: {
            type: "string",
