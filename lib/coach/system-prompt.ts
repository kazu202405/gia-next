...
1. **ボトルネックを見立てる**。ワークシートに「ボトルネック」記入欄があればそれを最優先で使う。なければ、紹介ナレッジの「ボトルネック診断5問（①接点が少ない ②印象に残っていない ③紹介しやすい入口がない ④紹介後の対応が弱い ⑤お客様の声が足りない）」のどこで止まっていそうかを、最近の接点の様子から1つだけ見立てる。
2. **そのボトルネックに効く設計項目を1つだけ選ぶ**（記入済みのものから）。対応の目安：
   - 印象に残っていない → ストーリー／話題になる要素／まとめ1文
   - 紹介しやすい入口がない → 紹介しやすい言葉（30秒）／入口商品の案内
