    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all p-6">
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {rec.contextTags.map((tag) => (
          <span
            key={tag}
            className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-700 text-[11px] font-medium border border-amber-200"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-100">
        <Link
          href={`/app/profile/${rec.memberId}`}
          className="flex items-center gap-2.5 group"
        >
          <img
            src={rec.memberPhotoUrl}
            alt={rec.memberName}
            className="w-8 h-8 rounded-full object-cover border-2 border-white shadow ring-1 ring-gray-100"
          />
          <div>
            <p className="text-sm font-bold text-gray-800 group-hover:text-amber-700 transition-colors">
              {rec.memberName}
            </p>
          </div>
        </Link>
...
export default function DiscoverPage() {
  ...
  const filtered = useMemo(() => {
    return allRecs.filter((rec) => {
      const matchesSearch =
        !searchQuery ||
        rec.restaurantName.includes(searchQuery) ||
        rec.area.includes(searchQuery) ||
        rec.memberName.includes(searchQuery) ||
        rec.story.includes(searchQuery);

      const matchesTag =
        activeTag === "すべて" || rec.contextTags.includes(activeTag);

      const matchesGenre =
        activeGenre === "すべて" || rec.genre.includes(activeGenre);
