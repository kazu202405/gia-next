...
function TreeNodeCard({
  ...
  return (
    <li className="relative">
      <div className="tree-node flex items-start gap-3 py-2">
        {/* Connector dot */}
...
function TreeStats() {
  ...
  const total = countNodes(referralTree) - 1; // exclude root
...
export default function TreePage() {
  ...
  useEffect(() => {
    ...
    const nodes = containerRef.current.querySelectorAll(".tree-node");
    gsap.fromTo(
      nodes,
      { opacity: 0, x: -20 },
      {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <ul className="space-y-0">
            <TreeNodeCard node={referralTree} depth={0} index={0} />
          </ul>
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-6 text-xs text-gray-400 px-2">
          ...
          <div className="flex items-center gap-2">
            <div className="w-5 h-0 border-t-2 border-amber-200" />
            <span>紹介の繋がり</span>
          </div>
        </div>
      </div>
