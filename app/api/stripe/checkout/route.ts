...
export async function POST() {
  ...
  // Stripe 未設定（env なし）や通信失敗で getStripeClient/getSalonPriceId が throw すると、
  // 生の 500（空ボディ）が返り、クライアントの res.json() が
  // 「Unexpected end of JSON input」になる。必ず JSON で「準備中」に倒す。
  ...
  try {
    ...
    console.error("[stripe.checkout] 作成失敗（Stripe設定/通信）:", err);
    return NextResponse.json(
