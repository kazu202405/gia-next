...
const DOMAIN_ADVICE: Record<string, { high: DomainAdviceLevel; mid: DomainAdviceLevel; low: DomainAdviceLevel }> = {
  ...
  習慣設計: {
    high: {
      ...
      evidence: "チャールズ・デュヒッグ著『習慣の力』によると、習慣のループ（きっかけ→行動→報酬）を可視化すると、その定着率は飛躍的に向上します。",
    },
    mid: {
    ...
    low: {
      ...
      evidence: "ジェームズ・クリアー著『Atomic Habits』によれば、小さな習慣の積み重ねは複利のように成果を生みます。毎日1%の改善で、1年後には37倍の差になる計算です。",
    },
  },
  コミュニケーション: {
    high: {
  ...
  リーダーシップ: {
    high: {
      ...
      evidence: "ダニエル・ピンク著『Drive』によると、自律性を与えられた社員は創造性とパフォーマンスが大幅に向上し、離職率も低下します。",
    },
    mid: {
  ...
  環境設計: {
    ...
    mid: {
      ...
      evidence: "カル・ニューポート著『Deep Work』によると、集中できる環境の有無が、知的労働の成果を2〜4倍左右するとされています。",
    },
    low: {
      ...
      evidence: "ドナルド・ノーマン著『誰のためのデザイン？』では、人がミスをするのは「人のせい」ではなく「環境のせい」。環境を変えれば行動は自然と変わると述べられています。",
    },
  },
};
